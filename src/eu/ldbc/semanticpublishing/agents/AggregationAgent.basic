...
//		this.turtleResultStatementsCounter = new TurtleResultStatementsCounter();
        ...
        logQueries = configuration.getBoolean(Configuration.LOG_QUERIES);

        logDir = Path.of("query_log", "aggregation-agent-" + (agentId++));
        try {
            if (logQueries) Files.createDirectories(logDir);
        } catch (IOException e) {
...
//			boolean drillDownQuery = false;
            ...
            resultCount = updateQueryStatistics(true, startedDuringBenchmarkPhase, aggregateQuery.getTemplateQueryType(), aggregateQuery.getTemplateFileName(), queryString, inputStreamResult, saveDetailedQueryLogs, queryId, System.currentTimeMillis() - executionTimeMs, timeStamp);
        ...
        if (logQueries && resultCount > 0) {
            try {
                Files.write(logDir.resolve("queryString-" + executionCount), queryString.getBytes());
                // Files.write(logDir.resolve("queryResult-" + executionCount), queryResult.getBytes());
            } catch (IOException e) {
                throw new RuntimeException(e);
            }
            executionCount++;
    ...
    private long updateQueryStatistics(boolean reportSuccess, boolean startedDuringBenchmarkPhase, QueryType queryType, String queryName, String queryString, InputStream inputStreamQueryResult, boolean useStringQueryResultOrInputStreamResult, long id, long queryExecutionTimeMs, String timeStamp) {
